...
export interface BudgetGate { reserve(operation: Operation, opts?: ReserveOptions): Promise<BudgetCapability>; commit(permit: BudgetCapability): Promise<void>; release(permit: BudgetCapability): Promise<void>; }
...
export function durableBudgetGate(env: Env, requestId: string): BudgetGate {
  ...
  const call = async (action: "reserve" | "commit" | "release", payload: unknown): Promise<unknown> => {
